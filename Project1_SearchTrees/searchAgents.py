    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                successor = ((nextNode, successorVisitedCorners), action, defaultCost)
...
def cornersHeuristic(state, problem):
    ...
    if len(notVisited) > 0:
        ...
        farthestPointCost = util.manhattanDistance(currentLocation, notVisited[0])
...
def foodHeuristic(state, problem):
    ...
    if len(foodList) > 0:
        ...
        for j in range(len(foodList)):
            # calculate distance between current state to corner
            foodLocation = foodList[j]
            lengthToFood = util.manhattanDistance(position, foodLocation)

            if lengthToFood < closestPointCost:
                closestPoint = j
                closestPointCost = lengthToFood
        ...
        for j in range(len(foodList)):
            ...
            if lengthToFood > farthestPointCost:
                farthestPoint = j
                farthestPointCost = lengthToFood
